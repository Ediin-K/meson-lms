...
export default function SubjectDetail() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true)
                const [subjectRes, modulesRes, enrollmentRes, groupsRes] = await Promise.all([
                    axiosInstance.get(`/subjects/${subjectId}`),
                    axiosInstance.get(`/subjects/${subjectId}/modules`),
                    axiosInstance.get(`/enrollments/user/${userId}`),
                    getSubjectGroups(subjectId)
                ])
                setSubject(subjectRes.data)
                setModules(modulesRes.data)
                setSubjectGroups(groupsRes)

                const owner = role === 'teacher' && subjectRes.data.teacherId === Number(userId)
                setIsOwner(owner)

                const enrolled = enrollmentRes.data.some(e => e.subjectId === Number(subjectId))
                setIsEnrolled(enrolled || owner)

                if (enrolled && !owner) {
                    progressService.getSubjectProgress(subjectId)
                        .then(r => setSubjectProgress(r.data))
                        .catch(() => {})
                }
            } catch (err) {
                console.error(err)
    ...
    const handleEnroll = async () => {
        try {
            ...
            await axiosInstance.post('/enrollments', {
                ...
                subjectGroupId: selectedGroupId ? Number(selectedGroupId) : undefined,
                subjectSubgroupId: selectedSubgroupId ? Number(selectedSubgroupId) : undefined,
    ...
    const selectedGroup = subjectGroups.find(group => group.id === Number(selectedGroupId))
    ...
    const handleModuleSubmit = async () => {
        try {
            if (moduleModal.editing) {
                ...
                await teacherContentService.createModule({ ...moduleForm, subjectId: Number(subjectId) })
            }
            const res = await axiosInstance.get(`/subjects/${subjectId}/modules`)
            setModules(res.data)
    ...
    if (!subject) {
        return (
            <Container maxWidth="lg" sx={{ mt: 6 }}>
                ...
                >
                    {t('subjectDetail.backToHome', 'Kthehu te ballina')}
                </Button>
            </Container>
        )
    }
    ...
    return (
        <section className="flex flex-col min-h-screen">
            <Container maxWidth="lg" className="grow py-8 px-4 sm:px-6 lg:px-8 mt-4 sm:mt-8">
                ...
                >
                    {t('subjectDetail.backToSubjects', 'Kthehu te lëndët')}
                </Button>
                        ...
                        <CardContent className="p-5!">
                            <div className="mb-3 flex items-center justify-between gap-2">
                                ...
                                <span className="text-sm font-black text-sky-600 dark:text-sky-400">
                                    {subjectProgress.viewedLessons}/{subjectProgress.totalLessons} leksione &nbsp;·&nbsp; {Math.round(subjectProgress.progressPercent)}%
                                </span>
                            </div>
                            {}
                            ...
                            <div className="flex flex-col gap-2">
                                {subjectProgress.modules.map((mod) => (
                                                    ...
                                                    >
                                                        ...
                                                        {lesson.lloji === 'QUIZ' && isEnrolled && !isOwner && (
                                                            <Box className="px-5 pb-4" onClick={(e) => e.stopPropagation()}>
                                                                <LessonQuizCard lessonId={lesson.id} subjectId={subjectId} compact />
                                                            </Box>
                                                        )}
                            ...
                            <div className="flex gap-3 mt-2">
                                ...
                                <Button
                                    ...
                                    disabled={
                                        !enrollmentKey.trim() ||
                                        enrolling ||
                                        (subjectGroups.length > 0 && !selectedGroupId) ||
