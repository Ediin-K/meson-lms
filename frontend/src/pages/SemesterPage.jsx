...
export default function SemesterPage() {
    ...
    useEffect(() => {
        const fetchSubjects = async () => {
            try {
                setLoading(true)
                const res = await axiosInstance.get(`/subjects/by-semester/${semesterId}`)
                ...
                {loading ? (
                    ...
                    <Box className="flex flex-col justify-center items-center py-20 px-4 text-center bg-white/60 dark:bg-slate-900/60 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700 shadow-sm">
                        ...
                        <Typography variant="h6" className="!font-bold !text-slate-800 dark:!text-slate-200">
                            {t('semester.empty.title', 'No subjects Found')}
                        </Typography>
                        <Typography variant="body2" className="!mt-2 !text-slate-500 dark:!text-slate-400 max-w-sm">
                            {t('semester.empty.desc', "We couldn't find any active subjects for this semester. Please check back later or contact your administrator.")}
                        </Typography>
                    </Box>
                ) : (
                    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        {subjects.map(course => (
                            <Link 
                                ...
                                key={course.id}
                                onClick={() => localStorage.setItem('lastSubjectId', course.id)}
