...
function StatCard({ label, icon: Icon, color, bg }) {
  return (
    <Card
      elevation={0}
      className="rounded-2xl border border-slate-200/80 bg-white dark:!bg-slate-900/60 dark:!border-slate-700/80 shadow-sm"
    >
...
const STAT_CARDS = [
  ...
  {
    label: "Regjistrime Totale",
    icon: AssignmentOutlinedIcon,
    color: "text-rose-600",
    bg: "bg-rose-100 dark:bg-rose-900/40",
  },
  {
    label: "Certifikata Lëshuara",
    ...
    <section className="flex flex-col min-h-screen">
      <Container maxWidth="lg" className="flex-grow py-8 mt-4 sm:mt-8">
        ...
        <Box className="mb-8">
          ...
          >
            {t(
              "home.admin.services.reports.desc",
              "Statistika të detajuara mbi performancën e platformës.",
            )}
          </Typography>
        </Box>
          ...
          <>
            ...
            <Grid container spacing={3}>
              <Grid item xs={12} md={7}>
                <Card
                  elevation={0}
                  className="rounded-2xl border border-slate-200/80 bg-white dark:!bg-slate-900/60 dark:!border-slate-700/80 shadow-sm h-full"
                >
                  <CardContent className="!p-6">
                    ...
                    <Typography variant="caption" className="!text-slate-500">
                      Numri i përdoruesve aktivë sipas muajit
                    </Typography>
                    {/* TODO: render chart from stats.activityByMonth */}
                    <Box className="flex items-end gap-3 mt-6 h-40">
                      {["Jan", "Shk", "Mar", "Pri", "Maj", "Qer"].map((m, index) => (
                        <div
                          key={m}
                          className="flex-1 flex flex-col items-center gap-1"
                        >
                          <div
                            className="w-full rounded-t-lg bg-slate-100 dark:bg-slate-800 animate-pulse"
                            style={{ height: `${[40, 70, 45, 60, 35, 80][index]}%` }}
                          />
                    ...
                    <p className="text-xs text-slate-400 text-center mt-3">
                      Të dhënat do të shfaqen pas lidhjes me API
                    </p>
                  </CardContent>
                </Card>
              </Grid>
                    ...
                    <Box className="flex flex-col gap-4 mt-5">
                      {[1, 2, 3, 4, 5].map((i) => (
                        <div key={i} className="flex items-center gap-3">
                          <span className="text-sm font-black text-slate-300 w-6">
                            #{i}
                          </span>
                          <div className="flex-1">
                            <div
                              className="h-3 rounded-full bg-slate-100 dark:bg-slate-800 animate-pulse mb-2"
                              style={{ width: `${100 - i * 12}%` }}
                            />
                            <div
                              className="h-1.5 rounded-full bg-slate-100 dark:bg-slate-800 animate-pulse"
                              style={{ width: `${80 - i * 8}%` }}
                            />
                          </div>
                        </div>
