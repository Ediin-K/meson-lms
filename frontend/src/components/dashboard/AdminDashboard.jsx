        ...
        return (
            <Box className="bg-white/90 dark:bg-slate-800/90 p-3 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl backdrop-blur-md">
                ...
                {payload.map((entry, index) => (
                    <Typography key={index} variant="body2" className="!font-black" style={{ color: entry.color || entry.fill }}>
...
export default function AdminDashboard() {
    ...
    const categoryData = [
        ...
        { name: t('home.admin.services.charts.design'), value: 30, color: '#0ea5e9' },
    ...
    const enrollmentStatusData = [
        { name: t('home.admin.services.charts.active'), students: 850, fill: '#0ea5e9' },
        { name: t('home.admin.services.charts.completed'), students: 420, fill: '#10b981' },
        { name: t('home.admin.services.charts.pending'), students: 150, fill: '#f59e0b' },
    ]
            ...
            <Box className="mb-10 rounded-[3rem] border border-slate-200/60 bg-white/80 p-6 shadow-2xl shadow-slate-200/20 sm:p-12 dark:!border-slate-700/60 dark:!bg-slate-900/50 dark:shadow-none">
                ...
                {!loading && stats ? (
                    <Box className="mt-10 grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            { label: t('home.admin.stats.users'), value: stats.totalUsers, icon: PeopleRounded, color: 'text-indigo-600', bg: 'bg-indigo-50 dark:bg-indigo-900/20' },
                            { label: t('home.admin.stats.courses'), value: stats.totalCourses, icon: SchoolRounded, color: 'text-sky-600', bg: 'bg-sky-50 dark:bg-sky-900/20' },
                            { label: t('home.admin.stats.enrollments'), value: stats.totalEnrollments, icon: AssignmentRounded, color: 'text-amber-600', bg: 'bg-amber-50 dark:bg-amber-900/20' },
                            { label: t('home.admin.services.certificates.title'), value: stats.totalCertificates, icon: WorkspacePremiumRounded, color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },
                        ].map((s, i) => (
                            <Box key={i} className="p-6 rounded-3xl bg-white dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 shadow-sm transition-transform hover:scale-[1.02]">
                                ...
                                <Typography variant="h4" className="!font-black dark:!text-white">{s.value}</Typography>
                            </Box>
                        ))}
                    </Box>
                ) : (
                ...
                {/* ANALYTICS SECTION */}
                <Typography variant="h5" className="!mt-16 !mb-8 !font-black !text-slate-800 dark:!text-white flex items-center justify-center gap-3">
                    ...
                    <Grid container spacing={4} justifyContent="center">
                        ...
                        <Grid item xs={12} md={4}>
                            <Card className="!rounded-[2.5rem] !border-none !bg-slate-50/50 dark:!bg-slate-800/30 !p-8 shadow-inner h-full">
                                <Typography variant="subtitle1" className="!font-black !mb-8 !text-slate-800 dark:!text-white uppercase tracking-widest text-center">
                                    ...
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={enrollmentStatusData}>
                                            ...
                                            <XAxis
                                                ...
                                                axisLine={false}
                                                tickLine={false}
                                                tick={{ fill: isDark ? '#94a3b8' : '#64748b', fontWeight: 800, fontSize: 12 }}
                                            />
                                            <YAxis hide />
