...
export default function ProfileSubjectsSection({ enrollments, t }) {
    ...
    <Card elevation={0} className="rounded-2xl border border-slate-200/80 bg-white dark:!border-slate-700/80 dark:!bg-slate-900">
      <CardContent className="!p-6">
        <Typography variant="subtitle1" className="!mb-4 !flex !items-center !gap-2 !font-bold !text-slate-900 dark:!text-white">
          ...
          {t('studentProfile.subjects')}
        </Typography>
        {!enrollments?.length ? (
          <Box>
            <Typography variant="body2" className="!text-slate-500">
              {t('studentProfile.nosubjects')}
            </Typography>
            <button
              type="button"
            ...
            >
              {t('studentProfile.browsesubjects')}
            </button>
          </Box>
        ) : (
          <div className="flex flex-col gap-3">
            {enrollments.map((enrollment) => (
              <Box
                ...
                role="button"
                tabIndex={0}
                onKeyDown={(e) => e.key === 'Enter' && navigate(`/subject/${enrollment.subjectId}`)}
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    {(enrollment.subjectGroupName || enrollment.professorName) && (
                      <Typography variant="caption" className="!mt-1 !block !text-slate-500">
                        {[enrollment.subjectGroupName, enrollment.professorName].filter(Boolean).join(' · ')}
                      </Typography>
                    )}
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <Chip
                        label={`${enrollment.subjectEcts ?? 5} ECTS`}
                        size="small"
